...
export default function App(){
 ...
 const[rapportsSite,setRapportsSite]=useState([]);const[intendance,setIntendance]=useState([]);const[suiviEduc,setSuiviEduc]=useState([]);const[purgeMarks,setPurgeMarks]=useState({});
...
useEffect(()=>{try{localStorage.setItem("pdsr_docs",JSON.stringify(docs));}catch{}if(!docsReadOK.current)return;if(docs.length===0&&docsWasNonEmpty.current)return;if(docs.length)docsWasNonEmpty.current=true;if(window._docsTimer)clearTimeout(window._docsTimer);window._docsTimer=setTimeout(()=>{fbSet("documents",docs);},1500);},[docs]);
  ...
  useEffect(()=>{if(!user)return;const sameIds=(a,b)=>{if(a.length!==b.length)return true;const s=new Set(a.map(x=>x&&x.id));return!b.every(x=>s.has(x&&x.id));};const tick=async()=>{if(!fbReadOK.current)return;let rem=null;try{const r=await fetch(FB_URL+"/data.json?auth="+FB_SECRET);if(r.ok)rem=await r.json();}catch(e){}if(!rem)return;const tomb=new Set();((rem.deletionLogs)||[]).forEach(t=>{if(t&&t.origId!=null)tomb.add(t.type+":"+t.origId);});const merge=(prev,remoteArr,typeName)=>{const m={};(Array.isArray(prev)?prev:[]).forEach(x=>{if(x&&x.id!=null)m[x.id]=x;});(Array.isArray(remoteArr)?remoteArr:[]).forEach(x=>{if(x&&x.id!=null&&!m[x.id])m[x.id]=x;});return Object.values(m).filter(x=>!tomb.has(typeName+":"+x.id));};setRapports(prev=>{const n=merge(prev,rem.rapports,"rapport");return sameIds(prev||[],n)?n:prev;});setEvenements(prev=>{const n=merge(prev,rem.evenements,"evenement");return sameIds(prev||[],n)?n:prev;});setAgenda(prev=>{const n=merge(prev,rem.agenda,"agenda");return sameIds(prev||[],n)?n:prev;});setIntendance(prev=>{const n=merge(prev,rem.intendance,"intendance");return sameIds(prev||[],n)?n:prev;});setSuiviEduc(prev=>{const n=merge(prev,rem.suiviEduc,"suiviEduc");return sameIds(prev||[],n)?n:prev;});if(rem.purgeMarks){setPurgeMarks(prev=>({...prev,...rem.purgeMarks}));const pm=rem.purgeMarks;if(pm.loginLogs)setLoginLogs(prev=>{const n=(prev||[]).filter(x=>(x&&x.date||"")>pm.loginLogs);return n.length===(prev||[]).length?prev:n;});if(pm.deletionLogs)setDeletionLogs(prev=>{const n=(prev||[]).filter(x=>(x&&x.date||"")>pm.deletionLogs);return n.length===(prev||[]).length?prev:n;});}if(rem.presences){setPresences(prev=>{const pm={};const add=(a)=>{(Array.isArray(a)?a:(a&&typeof a==="object"?Object.values(a):[])).forEach(p=>{if(p&&p.jeuneId!=null&&p.date)pm[p.jeuneId+"|"+p.date]=p;});};add(prev);add(rem.presences);const n=Object.values(pm);return(Array.isArray(prev)&&prev.length===n.length)?prev:n;});}if(rem.users&&user&&user.role!=="directeur"&&user.role!=="chef_service"&&!user.isAdmin){const fbu=(Array.isArray(rem.users)?rem.users:Object.values(rem.users)).filter(Boolean);setAppUsers(prev=>{const next=fbu.map(fu=>{const base=USERS.find(x=>x.id===fu.id)||fu;return{...base,...fu};});const changed=next.length!==(prev||[]).length||next.some(n=>{const p=(prev||[]).find(x=>x.id===n.id);return!p||p.role!==n.role||p.site!==n.site||p.disabled!==n.disabled;});return changed?next:prev;});}};const iv=setInterval(tick,5000);return()=>clearInterval(iv);},[user]);
...
useEffect(()=>{if(fbSkip.current){fbSkip.current=false;return;}if(!user)return;if(!fbLoaded.current)return;if(!fbReadOK.current)return;const presN=presences?(Array.isArray(presences)?presences.length:Object.keys(presences).length):0;const localVide=(!rapports||rapports.length===0)&&(!evenements||evenements.length===0)&&presN===0;if(localVide&&!fbWasNonEmpty.current){return;}if(window._fbTimer)clearTimeout(window._fbTimer);const _doSave=async()=>{window._fbTimer=null;
  ...
  const union=(loc,rem,typeName)=>{const m={};(Array.isArray(rem)?rem:[]).forEach(x=>{if(x&&x.id!=null)m[x.id]=x;});(Array.isArray(loc)?loc:[]).forEach(x=>{if(x&&x.id!=null)m[x.id]=x;});return Object.values(m).filter(x=>!tomb.has(typeName+":"+x.id));};
...
const collectData=()=>({rapports,presences,evenements,jeunes:appJeunes,users:appUsers,agenda,loginLogs,majeurs:appMajeurs,rapportsSite,djiPlan:appDjiPlan,fatPlan:appFatPlan,deletionLogs,projets,sejourConfig,etabConfig,_exportTs:new Date().toISOString()});
const restoreData=(d)=>{if(!d)return;
  ...
  if(d.presences)setPresences(Array.isArray(d.presences)?d.presences.filter(Boolean):(typeof d.presences==="object"?Object.values(d.presences).filter(Boolean):[]));
...
const checkIntegrity=async()=>{try{const d=await fbGet("data")||{};const loc=collectData();const keys=[["rapports","Rapports"],["presences","Présences"],["evenements","Événements"],["jeunes","Jeunes"],["majeurs","Majeurs"],["agenda","Agenda"],["projets","Projets"],["rapportsSite","Rapports de site"]];const cnt=v=>!v?0:Array.isArray(v)?v.length:Object.keys(v).length;return keys.map(([k,l])=>({label:l,local:cnt(loc[k]),cloud:cnt(d[k])}));}catch(e){return null;}};
  ...
  const TITLES={dashboard:"Tableau de bord",jeunes:"Jeunes","jeune-detail":sel?`${sel.prenom} ${sel.nom}`:"Fiche",rapports:"Rapports journaliers",presences:"Présences",evenements:"Événements indésirables","rapport-hebdo":"Rapport hebdomadaire",projets:"Projets personnalisés","rapport-site":"Rapport de site","espace-rh":"Espace éducateur"};
