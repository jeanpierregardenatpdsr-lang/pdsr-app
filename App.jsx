    ...
    <aside style={{position:"fixed",top:0,left:open?0:-270,width:260,height:"100vh",background:"linear-gradient(180deg,"+C.dark+" 0%,#2A1500 100%)",zIndex:50,transition:"left 0.3s cubic-bezier(0.4,0,0.2,1)",display:"flex",flexDirection:"column",overflowY:"auto",boxShadow:open?"8px 0 32px rgba(0,0,0,0.3)":"none"}}>
      ...
      <div style={{padding:"14px 18px",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{color:C.white,fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:6}}>{user.name}{user.isAdmin&&<span style={{background:C.gold,color:C.dark,fontSize:8,fontWeight:900,letterSpacing:"0.06em",padding:"2px 6px",borderRadius:5,textTransform:"uppercase"}}>Admin</span>}</div><div style={{color:C.sable,fontSize:11,fontWeight:600}}>{user.isAdmin?"Administrateur":user.role==="directeur"?"Directeur":user.role==="chef_service"?"Chef de service":user.role==="coordinateur_site"?`Coordinateur · ${user.site}`:`Éducateur · ${user.site}`}</div></div>
        </div>
      </div>
...
function Admin({users,jeunes,onUpdateUsers,onUpdateJeunes,loginLogs,appMajeurs,onUpdateMajeurs,deletionLogs,onPurgeLogs,onPurgeDeletionLogs,rapports,evenements,sejourConfig,onUpdateSejours,presences,onChangeP,agenda,onUpdateAgenda,projets,rapportsSite,onUpdateRapportsSite,onDeleteRapport,onUpdateRapport,onDeleteEvenement,onUpdateEvenements,currentUser,isAdmin,onViewAs,onForcePush,onForcePull,onCheckIntegrity,onBackup,onRestore,etabConfig,onUpdateEtab,onArchiveSejour}){
  ...
  const[opFilter,setOpFilter]=useState("");const[opSite,setOpSite]=useState("Tous");const[editRap,setEditRap]=useState(null);const[editRapText,setEditRapText]=useState("");const[editRapDate,setEditRapDate]=useState("");
 ...
 <div style={{marginTop:6}}><div style={{fontSize:11,fontWeight:700,color:C.light,marginBottom:4}}>Section</div><select style={{...S.inp}} value={newSection} onChange={e=>setNewSection(e.target.value)}><option value="mineurs">Mineurs</option><option value="majeurs">Majeurs</option></select></div></>}
    ...
    {tab==="op-rapports"&&(()=>{const list=(rapports||[]).filter(r=>matchOp(r.jeuneId,r.observation)).sort((a,b)=>(b.date||"").localeCompare(a.date||""));return(<div>
      ...
      {list.map(r=><div key={r.id} style={{...S.card,marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5,gap:8,flexWrap:"wrap"}}><div><span style={{fontWeight:800,fontSize:13,color:C.dark}}>{opName(r.jeuneId)}</span><span style={{fontSize:11,color:C.gold,fontWeight:700,marginLeft:8}}>{fmt(r.date)}</span>{r.author&&<span style={{fontSize:10,color:C.light,marginLeft:6}}>par {r.author}</span>}</div><div style={{display:"flex",gap:5}}>{editRap===r.id?<><button onClick={()=>{onUpdateRapport&&onUpdateRapport(r.id,{observation:editRapText,date:editRapDate||r.date});setEditRap(null);}} style={{padding:"4px 10px",borderRadius:6,border:"none",background:"#2E7D32",color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>✓</button><button onClick={()=>setEditRap(null)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+C.border,background:C.white,color:C.mid,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Annuler</button></>:<><button onClick={()=>{setEditRap(r.id);setEditRapText(r.observation||"");setEditRapDate(r.date||"");}} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+C.gold,background:C.goldLight,color:C.goldDark,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Modifier</button><button onClick={()=>{if(confirm("Supprimer ce rapport ?"))onDeleteRapport&&onDeleteRapport(r.id);}} style={{padding:"4px 10px",borderRadius:6,border:"1px solid #C62828",background:"#FFEBEE",color:"#C62828",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>×</button></>}</div></div>
        {editRap===r.id?<div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:10,fontWeight:800,color:C.light,textTransform:"uppercase"}}>Date du rapport</span><input type="date" value={editRapDate} onChange={e=>setEditRapDate(e.target.value)} style={{padding:"4px 8px",borderRadius:6,border:"1px solid "+C.border,fontSize:12,fontFamily:"inherit"}}/></div><textarea value={editRapText} onChange={e=>setEditRapText(e.target.value)} rows={3} style={{...S.inp,width:"100%",resize:"vertical",fontSize:12}}/></div>:<p style={{margin:0,fontSize:12,color:C.mid,lineHeight:1.5}}>{r.observation}</p>}
      </div>)}
    </div>);})()}
...
useEffect(()=>{if(fbSkip.current){fbSkip.current=false;return;}if(!user)return;if(!fbLoaded.current)return;if(!fbReadOK.current)return;const nonVide=(rapports&&rapports.length)||(evenements&&evenements.length)||(appJeunes&&appJeunes.length>0&&(rapports.length||evenements.length||Object.keys(presences||{}).length));const presN=presences?(Array.isArray(presences)?presences.length:Object.keys(presences).length):0;const localVide=(!rapports||rapports.length===0)&&(!evenements||evenements.length===0)&&presN===0;if(localVide&&!fbWasNonEmpty.current){return;}if(window._fbTimer)clearTimeout(window._fbTimer);window._fbTimer=setTimeout(()=>{const data={rapports,presences,evenements,jeunes:appJeunes,users:appUsers,agenda,loginLogs,majeurs:appMajeurs,rapportsSite,djiPlan:appDjiPlan,fatPlan:appFatPlan,deletionLogs,projets,sejourConfig,etabConfig};const dataVide=(!data.rapports||data.rapports.length===0)&&(!data.evenements||data.evenements.length===0);if(dataVide&&fbWasNonEmpty.current){console.warn("[PDSR] Envoi vide bloqué : la base contient des données, on n'écrase pas.");return;}fbSet("data",data);if(!dataVide)fbSet("backups/jour_"+new Date().toISOString().slice(0,10),{...data,_ts:new Date().toISOString()});},2000);},[rapports,presences,evenements,appUsers,appJeunes,agenda,loginLogs,appMajeurs,rapportsSite,appDjiPlan,appFatPlan,deletionLogs,projets,sejourConfig]);
...
const restoreData=(d)=>{if(!d)return;
  ...
  if(d.presences)setPresences(typeof d.presences==="object"&&!Array.isArray(d.presences)?d.presences:toArr(d.presences));
