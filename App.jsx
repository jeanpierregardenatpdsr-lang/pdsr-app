...
useEffect(()=>{(async()=>{let d=await fbGet("data");if(d){fbSkip.current=true;if(d.rapports)setRapports(Array.isArray(d.rapports)?d.rapports:Object.values(d.rapports));if(d.presences)setPresences(typeof d.presences==="object"&&!Array.isArray(d.presences)?d.presences:Array.isArray(d.presences)?d.presences:INIT_PRESENCES);if(d.evenements)setEvenements(Array.isArray(d.evenements)?d.evenements:Object.values(d.evenements));if(d.jeunes){const jArr=Array.isArray(d.jeunes)?d.jeunes:Object.values(d.jeunes);setAppJeunes(jArr.map(fj=>{const base=JEUNES.find(x=>x.id===fj.id)||fj;return{...base,...fj};}));}if(d.agenda)setAgenda(Array.isArray(d.agenda)?d.agenda:Object.values(d.agenda));if(d.loginLogs)setLoginLogs(Array.isArray(d.loginLogs)?d.loginLogs:Object.values(d.loginLogs));if(d.users)setAppUsers(prev=>{const fb=Array.isArray(d.users)?d.users:Object.values(d.users);return fb.map(fu=>{const base=USERS.find(x=>x.id===fu.id)||fu;return{...base,...fu};});});if(d.majeurs)setAppMajeurs(Array.isArray(d.majeurs)?d.majeurs:Object.values(d.majeurs));if(d.rapportsSite)setRapportsSite(Array.isArray(d.rapportsSite)?d.rapportsSite:Object.values(d.rapportsSite));if(d.djiPlan&&typeof d.djiPlan==="object")setAppDjiPlan(prev=>({...DJI_PLAN,...d.djiPlan}));if(d.fatPlan&&typeof d.fatPlan==="object")setAppFatPlan(prev=>({...FAT_PLAN,...d.fatPlan}));}})();},[]);
