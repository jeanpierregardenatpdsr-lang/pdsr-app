...
const saveLS=(rapports,presences,evenements,users,jeunes,agenda,loginLogs,majeurs,rapportsSite,djiPlan,fatPlan,deletionLogs)=>{try{localStorage.setItem(LS_KEY,JSON.stringify({rapports,presences,evenements,users,jeunes,agenda,loginLogs,majeurs,rapportsSite,djiPlan,fatPlan,deletionLogs,ts:Date.now()}));}catch{}};
...
function Rapports({user,rapports,onSave,onDelete,onUpdate,majeurs}){
  ...
  return(<div style={{padding:"18px 14px",maxWidth:700,margin:"0 auto"}}>
    ...
    {(()=>{const allR=(rapports||[]).filter(r=>vj.some(j=>j.id===r.jeuneId)).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,MAX_DISPLAY);const totalPages=Math.ceil(allR.length/PER_PAGE);const pageR=allR.slice(rPage*PER_PAGE,(rPage+1)*PER_PAGE);return(<>
...
function Admin({users,jeunes,onUpdateUsers,onUpdateJeunes,loginLogs,appMajeurs,onUpdateMajeurs,deletionLogs,onPurgeLogs,onPurgeDeletionLogs}){
 ...
 const[newSection,setNewSection]=useState("mineurs");
  ...
  return(<div style={{padding:"18px 14px",maxWidth:800,margin:"0 auto"}}>
    ...
    <div style={{display:"flex",gap:7,marginBottom:16}}>{[{k:"educs",l:"Éducateurs"},{k:"jeunes",l:"Jeunes"},{k:"majeurs",l:"Majeurs"},{k:"creds",l:"Identifiants"},{k:"logs",l:"Logs"}].map(t=><button key={t.k} onClick={()=>setTab(t.k)} style={{padding:"7px 16px",borderRadius:20,border:`1.5px solid ${tab===t.k?C.gold:C.border}`,background:tab===t.k?C.gold:C.white,color:tab===t.k?C.white:C.mid,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{t.l}</button>)}</div>
    {tab==="educs"&&<div>
  ...
  return(<div style={{fontFamily:"'Nunito',sans-serif",background:`linear-gradient(180deg,${C.sableLight} 0%,#F5EFE0 40%,#EDE4D0 100%)`,minHeight:"100vh",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column"}}>
      ...
      <main style={{flex:1,overflowY:"auto"}}>
        ...
        {page==="rapports"&&<Rapports user={user} rapports={rapports} onSave={addR} onDelete={delR} onUpdate={(id,field,val)=>setRapports(p=>p.map(r=>r.id===id?{...r,[field]:val}:r))} majeurs={appMajeurs}/>}
